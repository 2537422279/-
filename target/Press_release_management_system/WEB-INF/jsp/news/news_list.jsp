...
<div class="rightinfo">
    <div class="tools">
        <ul class="toolbar">
            <li class="click"><a href="${pageContext.request.contextPath}/toAddNews">发布新闻</a></li>
        </ul>
    </div>
    <form action="${pageContext.request.contextPath}/findNewsByPage" id="newsListForm"
          name="ff" method="post">
        <ul class="seachform">
            <li><label>&nbsp;&nbsp;&nbsp;&nbsp;搜索关键词:</label>
                <input name="keywords" value="${keywords}" placeholder="这里输入标题或关键词" type="text" style="width: 400px;" class="scinput" />
            </li>
            <li>
                <label style="width: 60px;">新闻类别：</label>
                <select
            ...
            <li>
                <label>&nbsp;</label><input name="" type="submit"  value="查询" />
            </li>
        </ul>
            ...
            <c:if test="${!empty pb.list}">
                <c:forEach items="${pb.list}" var="news">
                    <tr>
                        ...
                        <td align="center">
                            <a href="${pageContext.request.contextPath}/toEditNews?newId=${news.newId}">修改</a>
                            <c:if test="${news.publishStatus=='2'}" var="status_flag">
                                &nbsp;|&nbsp;<a href='#' onclick="upNews(${news.newId})">发布</a>
                            </c:if>
                            <c:if test="${not status_flag}">
                                &nbsp;|&nbsp;<a href='#' onclick="downNews(${news.newId})">撤稿</a>
                            </c:if>
                            &nbsp;    |&nbsp;<a href='#' onclick="delNews(${news.newId})">删除</a>
                        </td>
                    </tr>
                </c:forEach>
                <tr>
        ...
        <script type="text/javascript">
            ...
            {
                let form=document.getElementById("newsListForm");
            ...
            function validate()
            {
                ...
                let gotoPageNo = document.getElementById("gotoPageNo").value;
            ...
            function upNews(newsId){
                $.ajax({
                    url:"${pageContext.request.contextPath }/setNewsPublishStatus",
                    type:"post",
                    //data表示发送的数据
                    data:JSON.stringify({newId:newsId,publishStatus:1}),
                    // 定义发送请求的数据格式为JSON字符串
            ...
            function downNews(newsId){
                $.ajax({
                    ...
                    type:"post",
                    //data表示发送的数据
                    data:JSON.stringify({newId:newsId,publishStatus:2}),
                    // 定义发送请求的数据格式为JSON字符串
                    contentType:"application/json;charset=UTF-8",
                    //定义回调响应的数据格式为JSON字符串，该属性可以省略
                    dataType:"json",
            ...
            function delNews(newsId){
                if(window.confirm("您确定要删除吗？"))
                {
                    $.ajax({
                        ...
                        type:"post",
                        //data表示发送的数据
                        data:JSON.stringify({newId:newsId}),
                        // 定义发送请求的数据格式为JSON字符串
                        contentType:"application/json;charset=UTF-8",
                        //定义回调响应的数据格式为JSON字符串，该属性可以省略
                        dataType:"json",
